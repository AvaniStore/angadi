...
function renderBillRows() {
  const html = billItems.map((it, i) => `
    <div style="display:grid;grid-template-columns:1fr 70px 90px 70px 80px 32px;gap:8px;align-items:center;margin-bottom:6px" id="bill-row-${i}">
      ...
      <input type="number" min="0.01" step="1" value="${it.qty}" onchange="billSetQty(${i}, this.value)" style="padding:6px 8px;border:1px solid var(--border2);border-radius:var(--radius);font-size:13px;background:var(--bg2);color:var(--text);text-align:center;width:100%">
